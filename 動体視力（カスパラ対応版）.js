...
スキルのカスタムパラメータに{exAvoid:xxx}または{exAvoid:xxx, turnType:y}を指定します。（xxxは整数、yは0～2の整数）

説明：
・相手に間接攻撃を仕掛けられた時、カスパラの"exAvoid"に設定したxxxの値の分、回避率上昇
　※数値を設定していない場合は0とみなされます。負の値も一応設定可能です（回避率が減少します）。

・カスパラに"turnType"を設定した場合、回避率上昇のタイミングを変更
　　・"turnType"が0, または未設定：相手に間接攻撃を仕掛けられた時
　　・"turnType"が1：自分から間接攻撃を仕掛けた時
　　・"turnType"が2：間接攻撃に対して常に発動

カスパラ設定例：
・{exAvoid:20}
　→相手に間接攻撃を仕掛けられた時、回避率+20

・{exAvoid:30, turnType:1}
　→自分から間接攻撃を仕掛けた時、回避率+30

・{exAvoid:50, turnType:2}
　→間接攻撃に対して常に回避率+50
